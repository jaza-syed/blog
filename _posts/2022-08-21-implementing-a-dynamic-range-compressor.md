...
As an aside, I came across an interesting project [web-synth](web-synth) by github user [Ameobea](https://github.com/ameobea/) which is browser-based music production tool with a really interesting tech stack. It has a Typescript based frontend which uses the WebAudio API to call custom DSP code written in Rust and compiled to WebAssembly to maintain performance. Using this stack to make the demo seems like a good way of simultaneously building an accessible browser-based tool while also writing performant low-level code for the core algorithms.
